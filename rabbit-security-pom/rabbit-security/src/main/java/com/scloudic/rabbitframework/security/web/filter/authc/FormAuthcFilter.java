 ...
 * @since 3.3.1
 */
public class FormAuthcFilter extends FormAuthenticationFilter implements RabbitSecurityFilter {
    private boolean frontEndSeparate = true;

    @Override
    protected void redirectToLogin(ServletRequest request, ServletResponse response) throws IOException {
        String loginUrl = getLoginUrl();
        RedirectUtils.redirect(request, response,
                loginUrl, StatusCode.SC_PROXY_AUTHENTICATION_REQUIRED, frontEndSeparate);
    }
